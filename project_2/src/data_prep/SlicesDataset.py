...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # <YOUR CODE GOES HERE>
        image_volume = self.data[data_idx]['image'][slc[1],:,:]
        image_tensor= torch.unsqueeze(torch.tensor(image_volume),dim=0)                      
        label_volume = self.data[data_idx]['seg'][slc[1],:,:]
        label_tensor =torch.unsqueeze(torch.tensor(label_volume),dim=0)
